...
import { useGetRightBanner } from "@/src/utils/banner/userRightBanner";
import { Trash2 } from "lucide-react";
import { useState, useRef, useEffect } from "react";
import toast from "react-hot-toast";

const RightBanner = () => {
  const { rightbanner, loading, error, refetch } = useGetRightBanner();
  const [banners, setBanners] = useState([]);
  useEffect(() => {
    if (rightbanner) {
      if (Array.isArray(rightbanner)) {
        setBanners(rightbanner);
      } else {
        setBanners([rightbanner]);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingBanner) {
        res = await RightBannerUploade(data, editingBanner._id || editingBanner.id);
        ...
        res = await RightBannerCreate(data);
        ...
        refetch();
        toast.success("âœ… Banner Create successfully!");
      }

      closeModal();
    } catch (error) {
      console.error("rightbanner save error:", error);
  ...
  const deleteBanner = async () => {
    try {
      ...
      await RightBannerDelete(deleteModal);
      setDeleteModal(null);
      refetch();
            ...
            <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-sm border border-gray-700/50 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="p-8 border-b border-gray-700/50">
                <h2 className="text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                  {editingBanner ? "âœï¸ Edit Banner" : "âœ¨ Create New Banner"}
                </h2>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  >
                    {editingBanner ? "ðŸ’¾ Update Banner" : "âœ¨ Create Banner"}
                  </button>
                </div>
              </form>
            </div>
