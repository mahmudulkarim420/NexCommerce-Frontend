...
const HomeSliderPage = () => {
  const { homebanner, loading, error, refetch } = useHomeBanner();
  ...
  const toggleSliderStatus = async (id) => {
    setSliders((prev) =>
      prev.map((slider) => (slider._id === id ? { ...slider, active: !slider.active } : slider)),
    );
              ...
              {sliders?.length > 0 ? (
                <div className="relative rounded-xl overflow-hidden bg-gray-800 shadow-xl">
                  <div className="aspect-[2/1] relative group">
                    <img
                      src={
                        sliders[currentSlide]?.images ||
                        "/placeholder.svg?height=400&width=800&query=slider preview"
                      }
                    ...
                    <div className="absolute bottom-6 left-6 right-6 text-white transform transition-transform duration-500 group-hover:translate-y-[-4px]">
                      <h4 className="text-2xl font-bold mb-3 text-balance">
                  ...
                  ?.map((slider, index) => (
                    ...
                    >
                      <img
                        src={
                          slider?.images ||
                          "/placeholder.svg?height=80&width=120&query=slider thumbnail"
                        }
                        alt={slider?.title}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-300 text-sm mb-3 text-pretty">
                          {slider?.Description}
                        </p>
