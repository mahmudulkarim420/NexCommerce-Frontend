...
const AddCategoriesComponent = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "name") {
      setFormData((prev) => ({
        ...
        metaTitle: newValue ? `${newValue} - NexCommerce` : "",
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  const toggleCategoryStatus = async (category) => {
    try {
      setCategories((prev) =>
        prev.map((cat) => (cat._id === category._id ? { ...cat, isActive: !cat.isActive } : cat)),
      );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {allsubCategorydata && allsubCategorydata.data ? (
                  <p className="text-white text-2xl font-bold">{allsubCategorydata.data.length}</p>
                ) : (
                    ...
                    <div className="flex items-center space-x-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-4 flex-wrap">
                          ...
                          <span className="text-gray-300 text-sm">Order: {category.sortOrder}</span>
                    ...
                    category.subcategories.length > 0 && (
                      <div className="mt-4 ml-8 space-y-2">
                        ...
                        {category.subcategories.map((subcat) => (
                          <div
                            key={subcat.id}
                            className="flex items-center justify-between bg-white/5 rounded-lg p-3 border border-white/10 hover:bg-white/10 transition-all duration-300"
                          >
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className=" flex gap-4 items-center justify-between">
                  ...
                  {/* Products Count */}
                  <div className="">
                    <div className="text-white font-semibold">{category.isActive}</div>
                    <div className="text-sm text-gray-400">
                      {" "}
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                ...
                {category.subcategories && category.subcategories.length > 0 && (
                  <div className="ml-6 mt-2 space-y-1">
                    {category.subcategories.map((subcat) => (
                      <div
                        key={subcat.id}
                        className="flex items-center space-x-3 p-2 bg-white/3 rounded-lg"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 rounded-xl p-6 border border-white/10 hover:from-blue-500/20 hover:to-cyan-500/20 transition-all duration-300">
              ...
              <p className="text-gray-300 text-sm mb-4">
                Use predefined category templates for quick setup
              </p>
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-xl p-6 border border-white/10 hover:from-green-500/20 hover:to-emerald-500/20 transition-all duration-300">
              ...
              <p className="text-gray-300 text-sm mb-4">
                View detailed analytics for each category
              </p>
