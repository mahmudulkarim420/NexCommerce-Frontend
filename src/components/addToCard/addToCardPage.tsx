...
import { useGetSubcategory } from "@/src/utils/useSubcategory";
import {
  AlertCircle,
  ArrowRight,
  Clock,
...
import { addToWishlistApi, removeFromWishlistApi } from "../../hook/useWishlist";

const ShoppingCartComponent = () => {
  const dispatch = useDispatch();
  const { items: rawItems = [], loading, error } = useSelector((state: any) => state.cart);
  ...
  const cartItems = rawItems.map((item) => {
    const product = item.productId || {};
    const id = product._id || item.productId_id;
    const categoryId = product.category?.[0]?._id || product.category?._id || null;
  ...
  const cartCategoryIds = useMemo(() => {
    return [...new Set(cartItems.map((item) => item.categoryId).filter(Boolean))];
  }, [cartItems]);
  ...
  const suggestedItems = useMemo(() => {
    ...
    return subcategory.filter(
      (item) => cartCategoryIds.includes(item.category?._id) && !cartProductIds.includes(item._id),
    );
                    ...
                    <div>
                      ...
                      <p className="text-sm text-teal-700">
                        {shipping === 0
                          ...
                          : `Add à§³${(2000 - subtotal).toLocaleString()} more to get free shipping`}
                      </p>
                    </div>
                  </div>
                </div>
